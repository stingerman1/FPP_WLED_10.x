...
State, preset, playlist, custom-palette, native-command, native-snapshot, auth and configuration files live outside immutable release trees. The installer builds before switching the runtime symlink. It never restarts FPP automatically. Unknown ownership prevents ambient output while FPP reloads the plugin.

The palette coordinator validates native gradient documents and calls the upstream FastLED converter through the Linux C ABI. It prepares complete 16-color tables before writing persistent settings, then publishes into pre-reserved renderer storage. Fixed previews read the pinned upstream tables; dynamic palettes use the upstream color tokens. Custom IDs descend from 200 independently of fixed palette names. Missing slots preserve index positions with gray renderer/UI placeholders while state validation rejects selecting missing slots.
