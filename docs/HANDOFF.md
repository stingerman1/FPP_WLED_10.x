...
Runtime-token access without SSH is implemented in e3665b9. The FPP plugin page explains privileges and confirms before retrieving the existing token; masked display, Show/Hide, Copy with manual fallback, Clear, and browser login are available. runtime-token.php inherits FPP access controls and requires POST plus a custom header, rejects cross-site fetches, uses no-store, never rotates/creates tokens and reports missing auth.json. No token is embedded in initial HTML, URL, localStorage or server logs. Access to the FPP page now explicitly grants token retrieval. Documented in INSTALL.md.

117 local tests passed, including PHP HTTP negative/access checks. Playwright synthetic-token tests verified cancel, retrieve, show, copy and login/clear. CI 34217124356 passed x86-64 and ARM64. Updated idle Pi 10.0.0.124 through the FPP plugin updater; rc=0 with runtime health and Apache syntax checks passing. Verified live wrapper retrieval and API login using the token only in memory (not printed or saved). No FPP restart or lighting-state changes performed. Browser fixture stopped and session closed.

Live 64-bit Pi 4 at 10.0.0.124 (stock FPP 10.0, reference 370e62ed7, Debian 13) exposed another installer failure: ctypes loading libfpp.so outside fppd fails on unresolved runMainFPPDLoop. Commit eb0215d replaces dlopen with conservative objdump extraction/comparison of the five compiled ABI constants. Unknown instruction encodings fail closed; FPP still performs final load validation. Regression fixture includes unresolved daemon symbols and a trapping constructor to prove nothing is loaded. All 116 local tests and x86-64/ARM64 CI run 34178175680 passed.

Applied eb0215d through POST /api/plugin/FPP_WLED_10.x/upgrade after confirming idle FPP. Upgrade completed rc=0; runtime health and Apache config test passed. Verified HTTP 200 on /fpp-wled/, /fpp-wled/settings, /fpp-wled/base.js, /fpp-wled/json/si and /fpp-wled/api/status; native /wled/ remains 200. Ambient remains disabled; observer_healthy is false pending FPP loading/restart. No FPP restart, output configuration, ambient enable or physical lighting test was performed. This confirms installation/web routing on one real Pi, not complete hardware acceptance.

Live diagnosis at 192.168.50.250: FPP main/plugin wrapper and native `/wled/` return 200; plugin route returns 404. Read-only inspection of `api/file/logs/fpp_plugin_manager.log` shows all installation attempts abort compiling the adapter: sizeof(void*) is 4 and the 64-bit assertion fails. Thus no runtime/proxy was installed. This is a 32-bit userspace/toolchain under a kernel that passed aarch64 detection. Added early Python/compiler/libfpp ELF architecture checks and a PHP bitness message hiding runtime links on 32-bit installations. Mixed-architecture regression test passes. No remote OS changes, installs or restarts were performed. Actual resolution requires supported 64-bit FPP userspace or a separately scoped 32-bit port; do not claim another route change fixes this Pi.

Correction after a reported Settings 404: FPP already owns `/wled/`. Plugin access is now `/fpp-wled/` (setup `/fpp-wled/settings`). The Apache integration test now uses a virtual host with a separate native `/wled/` page and verifies it remains reachable. The plugin page probes its runtime route and reports missing proxy configuration or unavailable service. Reinstall is required to replace the prior Apache configuration. The reported Pi at 10.0.0.161 was unreachable from the development environment; its installed state has not been verified.

Latest change: plugin links now use `/fpp-wled/` and `/fpp-wled/settings` on FPP's current origin, including custom ports and HTTPS. A plugin-owned Apache configuration proxies HTTP/WebSocket traffic to authenticated `/run/fpp-wled/web.sock`, separate from trusted command IPC. Setup, schedules, palette downloads and notices preserve the prefix; upstream WLED already detects proxy paths. New installations bind the optional TCP listener to loopback. Existing configuration is preserved. Installer validates/reloads Apache and restores its prior config on failure; removal disables the route. Run the installer again when updating an existing Pi to install this route. A rollback to older runtime code requires direct port access. Verification: 113 Python tests pass, including real Apache HTTP/assets/login/WebSocket integration and custom-origin URL checks; renderer build, sanitized IPC and JS/shell syntax checks pass. Actual Pi installation remains unverified. The plugin repository is now public (user changed visibility).
