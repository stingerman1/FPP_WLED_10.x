...
Latest change: plugin links now use `/wled/` and `/wled/settings` on FPP's current origin, including custom ports and HTTPS. A plugin-owned Apache configuration proxies HTTP/WebSocket traffic to authenticated `/run/fpp-wled/web.sock`, separate from trusted command IPC. Setup, schedules, palette downloads and notices preserve the prefix; upstream WLED already detects proxy paths. New installations bind the optional TCP listener to loopback. Existing configuration is preserved. Installer validates/reloads Apache and restores its prior config on failure; removal disables the route. Run the installer again when updating an existing Pi to install this route. A rollback to older runtime code requires direct port access. Verification: 113 Python tests pass, including real Apache HTTP/assets/login/WebSocket integration and custom-origin URL checks; renderer build, sanitized IPC and JS/shell syntax checks pass. Actual Pi installation remains unverified. The plugin repository is now public (user changed visibility).
