...
The latest increment implements local sunrise/sunset scheduling and a Follow the daylight setup editor. Solar calculations use pinned `astral==3.2`; location requires latitude/longitude and an IANA timezone. Clock and solar timers support weekday selection, solar offsets, DST rules, polar-event skips, and persistent deduplication in `timers-fired.json`. The editor previews upcoming events, applies schedules without restarting, rejects stale revisions, and preserves unrelated pending configuration and raw-editor drafts. Show-owned/missed events remain skipped. Main files: `runtime/timers.py`, `runtime/schedules.py`, `runtime/config.py`, `web/schedules.js`, and `tests/test_schedules.py`. Local verification passed all 102 tests plus renderer, sanitized IPC and syntax checks; browser checks cover preview/save/reload, preserved drafts, desktop/mobile layout, and no horizontal overflow. Screenshots: ignored `output/playwright/schedules-desktop.png` and `schedules-mobile.png`. Next lighting priorities: nightlight sunrise animation mode, custom transition styles, and remaining portable effects. Legacy HTTP and preset-reference chains remain separate work.
...
The preceding implementation commit was **`671bd31`**: nightlight modes 0/1/2, RGBW lighting controls and bounded live pixel preview. Its 90 tests and x86-64/ARM64 CI checks passed. The renderer-port pin is unchanged; this increment adds Python/UI integration only. Check the current `main` workflow for the schedule increment.
...
2. Nightlight sunrise animation mode (0/1/2 now implemented) and custom transition styles. Sunrise/sunset scheduling is now implemented.
