...
2. On the FPP plugin page, use **Get runtime token**, read the notice and confirm. Copy the token into setup or choose **Save runtime access** for this browser. Access survives browser and runtime restarts, is remembered for one year and renews during access checks. WLED Settings displays the saved access status and hides the token field; **Forget access on this browser** removes it. Clearing browser data or using another browser/address requires login again. The existing token is retrieved without rotating it; SSH is not required. Anyone with access to the FPP page can retrieve it, so restrict FPP access to trusted users. Alternatively, read `auth.json` as the `fpp` user from `/home/fpp/media/config/plugin.FPP_WLED_10.x`. The HTTP cookie is HttpOnly/SameSite Strict; API clients use `Authorization: Bearer TOKEN`. HTTP is intended for the trusted show network; it does not provide transport encryption.
