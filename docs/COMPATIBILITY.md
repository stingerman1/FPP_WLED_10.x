...
| Timers | Linux local weekday/hour/minute/preset schedules. Local nightlight modes 0/1/2: delayed brightness, brightness fade, selected-segment RGBW color fade. Countdown pauses through shows/quiet periods; restart leaves it stopped. Clock/sunrise/sunset preset schedules, offsets, IANA timezones/DST, polar-event skips, persistent deduplication, and preview/edit UI are implemented. Nightlight mode 3 uses upstream Sunrise effect 104 for 1-60 minute sunrise/sunset animations, with phase pause during shows and prior-effect restoration. |
