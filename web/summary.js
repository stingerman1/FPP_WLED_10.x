...
(() => {
 ...
 let effects = [], presets = {}, nextCatalog = 0, fetching = false, latest;
 ...
 function render(data) {
  ...
  badge.dataset.state=state; badge.textContent=label; $('summaryDetail').textContent=detail;
 ...
 window.wledSummary = data => {
  ...
  Promise.all([wledFetch('/json/effects'),wledFetch('/presets.json')]).then(async responses=>{
   if(!responses.every(r=>r.ok))return;
   [effects,presets]=await Promise.all(responses.map(r=>r.json()));if(latest)render(latest);
