async(page)=>{
 ...
 for(const id of [57,6,11]){
  ...
  await page.waitForFunction(name=>document.querySelector('#linux-active-palette strong').textContent==='Palette: '+name,names[id]);
  ...
  if(state.seg[0].col[0][0]!==255||state.seg[0].col[0][1]!==160)throw Error('Palette selection overwrote effect colors');
  const main=await page.locator('.active-palette-strip').evaluate(el=>el.style.background);
 ...
 await page.waitForFunction(()=>document.querySelector('#linux-active-palette strong').textContent==='Palette: Candy');
 ...
 for(const theme of ['light','dark']){
  ...
  await page.locator('#linux-active-palette').scrollIntoViewIfNeeded();
 ...
 await settings.waitForFunction(()=>document.getElementById('summaryPalette').textContent==='Custom palette 0'&&document.querySelector('#summaryPalette .summary-palette-strip')?.style.background.includes('gradient'));
  await page.waitForFunction(()=>document.querySelector('#pallist .selected')?.dataset.id==='200'&&document.querySelector('.active-palette-strip').style.background.includes('gradient'));
 await page.request.post(base+'/api/palettes',{data:{slot:0,palette:[0,'00FF00',255,'00FFFF']}});
 await page.waitForFunction(()=>document.querySelector('.active-palette-strip').style.background.includes('0, 255, 0'));
